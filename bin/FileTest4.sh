...
F_num=4			#该FileTest?.sh 为第几个
#numt=$2
#testdir=`cat ${cpath}/conf/.tmptestdir${2}`
testdir=$2
#echo "testdir $2"
#testfile="java -jar ${cpath}/lib/FileTest.jar"
filesh=FileTest4.sh        #当前脚本名，用于日志打印和记录
threadnum=$threadnum4	   #F 的线程数
filesize=${filesize4} 	  #F 的文件 大小
recsize=${recsize4}	  #F 的文件块大小
ftelog=${testdir}/log/filetesterror.log  #FileTest 错误日志记录
ftlog=${testdir}/log/ftlog4_${filesize}_${recsize}.log #F 日志记录
ftpath=${testdir}/ft4_${filesize}x${threadnum}_${recsize} #F的测试目录
#filesh=$3  
#threadnum=$4
#ftelog=$5
#ftlog=$6
