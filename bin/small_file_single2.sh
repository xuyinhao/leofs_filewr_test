...
filesh=small_file_single2.sh
smfselog=${testdir}/log/smallf_singleerror.log
smfslog=${testdir}/log/smfslog2.log
writepath=${testdir}/smf_single2
pwdp=`pwd`
function init {
mkdir -p ${testdir}/log
for i in $(seq 1 $fileNum_sd)
do
rm -rf $writepath/smf_single2_${i}.txt &
done
wait
...
function run {
cd $testdir
valw=`$writefile smf_single2  1 $smsfile2 $smsrec2 100 ${fileNum_sd2} 2>&1`
flag=$?
...
function check {
cd $testdir
valr=`$writefile smf_single2 2 $smsfile2 $smsrec2 100 ${fileNum_sd2} 2>&1`
flag=$?
